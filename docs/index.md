...
## 9.2. İletişim Desenleri

-   **Sinyal/Slot (PyQt):** Kullanıcı arayüzünden gelen kullanıcı eylemlerini (düğme tıklamaları, slider değer değişiklikleri) `MainWindow`'a (Controller) bildirmek için kullanılır. Bu, arayüz (View) bileşenlerinin iş mantığından tamamen habersiz olmasını sağlar, böylece tasarım ayrımı korunur.
-   **Gözlemci Deseni (VTK):** **VTK**'daki 3D sahne etkileşimlerini yakalamak için kullanılır. `MainWindow`, 3D görselleştirme penceresi (`QVTKRenderWindowInteractor`) üzerindeki belirli olayları (fare tıklaması, fare hareketi, klavye olayları) `AddObserver` metodu ile "dinler" ve bu olaylara göre 3D araçlarını (Model) çalıştırır.

# 10. Çekirdek Alan Modelleri

| Model | Açıklama |
|---|---|
| `AnatomySession` | Mevcut mesh referansı, basınç sözlüğü, geri alma yığını, meta verileri (uzuv tarafı, zaman damgası) bir araya getirir. |
| `Pressure Dict` | `Dict[int, float]` nokta ID'sinden basınç değerine (sınırlandırılmış aralık, örn. -100..100) eşleme. |
| `Undo Stroke Entry` | `{ point_ids: list, old_values: list, new_values: list, brush_meta }` |
| `Landmark` | `{ id, name, point_id, 3D coordinates }` |
| `Cutting Pattern` | Türetilmiş poliline/poligon geometri referansı (**vtkPolyData** alt kümesi). |

# 11. Hizmet Katmanı: VTKProcessor

**VTKProcessor**, arka uçtaki 3D işleme araçları ile `MainWindow` (Controller) arasında bir arayüz görevi görür. Uzun süreli veya karmaşık 3D işlemlerin Controller'ı bloke etmeden asenkron olarak çalıştırılmasını sağlar.

**Sorumluluklar:**
-   **Dosya Girdi/Çıktısı (I/O):** Mesh dosyalarını (.stl, .ply) okuma ve yazma işlemlerini ilgili VTK okuyucularına/yazıcılarına yönlendirir.
-   **Mesh Dönüşümleri:** `vtkCleanPolyData` (temizleme), `vtkSmoothPolyDataFilter` (yumuşatma - çeşitli varyantlar), `vtkDecimatePro` (basitleştirme), `vtkLoopSubdivisionFilter` (alt bölümlere ayırma) gibi genel mesh işleme algoritmalarını yürütür.
-   **Tutarlı Sinyal Sözleşmesi:** Uzun süren her işlem için standart bir sinyal sözleşmesi sağlar: `processing_started` (işlemin başladığını bildirir), `progress_updated` (işlem ilerlemesini günceller), `processing_finished` (işlem tamamlandığında sonucu ve meta verileri döndürür), `log_message` (durum veya hata mesajlarını günlüğe kaydeder).
-   `original_mesh` **Anlık Görüntüsü:** `original_poly_data`'nın güvenli bir kopyasını tutarak, herhangi bir zamanda orijinal mesh durumuna geri dönme veya değişiklikleri buna göre sıfırlama olanağı sunar.

**Davranış Kuralları:**
-   Maliyetli çağrıları (`start`/`finish` sinyalleriyle sarmalar.
-   Her eylem için açıklayıcı günlük mesajları yayınlar.
-   Hafızadaki `original_poly_data` ve `active_poly_data` arasında veri tutarlılığını sağlamak.

# 12. Arka Uç Araçları (Backend Tooling)

`backend/` klasörü altında yer alan bu modüller, **Clonify Labs** yazılımının temel 3D işleme ve modelleme yeteneklerini sağlar. Her bir grup, ilgili operasyonları ve algoritmaları içerir.

| Araç Grubu | Sorumluluklar |
|---|---|
| Basınç Araçları (`pressure_tools.py`) | Fırça mantığı, skaler değerlerin meshe atanması (bake etme), ısı haritası oluşturma (renklendirme). |
| Kalıp Üretici (`male_mold_generator.py`) | Basınç değerlerine göre mesh geometrisini deforme etme (warp etme), fiziksel kalıp için nihai 3D modeli oluşturma. |
| Kesme Deseni (`pattern_cut.py`) | Mesh üzerinde düzlemsel veya parametrik eğriler belirleyerek kesme desenleri oluşturma, kesilen yüzeyleri çıkarma veya birleştirme. |
| İşaretleme Araçları (`landmark_tools.py`) | Anatomik işaret noktalarını 3D uzayda seçme ve yerleştirme, bu noktalardan yola çıkarak yönelim çerçevelerini (orientation frames) hesaplama. |
| Kamera Araçları (`camera_tools.py`) | Önceden tanımlanmış anatomik yönelimleri (örn: ön, yan, üst görünüm) ayarlama, kamera konumlarını kaydetme ve yükleme. |
| GPU Hızlandırma (`gpu_acceleration.py`) | Performans kritik operasyonlar için isteğe bağlı GPU işlem yolları (örn: NumPy/VTK'nın GPU filtrelerini kullanma), GPU mevcut değilse sorunsuz bir şekilde CPU'ya düşüş (fallback). |
| Performans Optimizasyonları (`performance_optimizations.py`) | Bulucuların (locator) yeniden kullanımı, hızlı veri erişimi, operasyonların gruplanması (batching), hızlı güncelleme taleplerinin kısıtlanması (throttling), render çağrılarının birleştirilmesi. |
| Şekillendirme / Düzleştirme (`flatten.py`) | Mesh manipülasyonu için çeşitli yumuşatma varyantları, geometrinin normalizasyonu veya düzleştirilmesi operasyonları. |

**Tasarım Prensibi:** Tek bir modül, ilgili operasyonları tutarlılık için mikro-modülerizasyon yerine bir arada tutar.

# 13. Asenkron İşleme

Performans kritik ve uzun süren işlemlerin (örn: büyük meshlerde ısı haritası hesaplama veya kalıp deformasyonu) kullanıcı arayüzünü dondurmasını önlemek için asenkron (eşzamansız) işleme kullanılır.

**AsyncMeshProcessor (QThread):**
-   **PyQt**'nin `QThread` sınıfından türetilen bu sınıf, arka planda ayrı bir iş parçacığında çalışır.
-   Ana thread'den (UI) bir operasyon anahtarı (örn: 'heatmap', 'mold') alır.
-   İçindeki `_process_<op>` metotlarına yönlendirerek işlemi yürütür.
-   İşlem tamamlandığında, `processing_complete` sinyalini sonuç yüküyle (payload) birlikte ana thread'e geri gönderir.

**Thread-Safety (İş Parçacığı Güvenliği):**
-   **VTK** nesneleri (**vtkPolyData**) doğrudan aktif renderer'a bağlıyken thread'de manipüle edilmez. Bunun yerine, derin kopyalar üzerinde çalışılır.
-   Sonuçlar (işlenmiş **PolyData** veya skaler diziler), ana thread'e geri gönderilir ve burada güvenli bir şekilde `active_poly_data`'ya uygulanır.
-   Özellikle skaler dizilerin güncellenmesi gibi işlemler, ana thread'de tamamlanma sinyali alındıktan sonra gerçekleştirilir.

# 14. Render (Görselleştirme) Hattı

Uygulamanın 3D görselleştirme hattı, **VTK**'nın standart render mekanizmasını kullanır ve performansı optimize etmek için belirli stratejiler içerir.

**Aktör ve Mesh İlişkisi:**
-   3D sahnede görselleştirilen her bir model, bir `vtkActor` nesnesi tarafından temsil edilir.
-   Bu aktör, görselleştirilecek 3D geometriyi sağlayan bir `vtkMapper` nesnesine referans verir.
-   `vtkMapper` ise, **vtkPolyData**'dan gelen geometrik ve skaler verileri görselleştirme için yorumlar.

**Skaler Dizi Konfigürasyonu:**
-   Farklı görsel durumlar için özel olarak adlandırılmış skaler diziler kullanılır:
    -   `PressureValues`: Mesh üzerindeki basınç değerlerini temsil eder. Isı haritası görselleştirmesi için kullanılır.
    -   `MoldDisplacement`: Kalıp oluşturma işleminde mesh'in ne kadar deplase edildiğini gösterir.
-   Bu skaler diziler, `vtkLookupTable` (LUT) ile ilişkilendirilir. LUT'ler, skaler değer aralıklarını renklere dönüştürerek ısı haritası gibi görselleştirmeleri mümkün kılar. Basınç değerleri için "diverging" (ayrışan) renk haritaları (örn: soğuktan sıcağa), deplasman için ise "ramp" (rampa) renk haritaları kullanılabilir.

**Render Güncellemeleri:**
-   Skaler dizilerdeki değişiklikler, otomatik olarak `vtkMapper`'ın renk aralığını ayarlar.
-   `BatchRenderer` (varsa), hızlı ardışık render taleplerini birleştirir ve daha az sayıda gerçek render çağrısı yayınlar. Bu, kullanıcı etkileşimi sırasında (örn: fırça ile boyama) arayüzün daha akıcı kalmasını sağlar, çünkü her küçük değişiklik hemen ayrı bir render çağrısını tetiklemez.
-   `vtkRenderWindow.Render()` çağrısı, sahnenin ekranda yeniden çizilmesini tetikler.
